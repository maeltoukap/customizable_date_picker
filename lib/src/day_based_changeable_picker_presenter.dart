...
class DayBasedChangeablePickerPresenter {
  ...
  void changeMonth(DateTime newMonth) {
    ...
    // DateTime curMonthFormat =
    //     DatePickerUtils.addMonthsToMonthDate(firstDate, monthPage);
    // DateTime curMonth =
    //     DateTime.parse(DateFormat("MMM").format(curMonthFormat));
    DateTime nextMonth =
        DatePickerUtils.addMonthsToMonthDate(firstDate, monthPage + 1);

    String prevMonthStr = localizations.formatMonthYear(prevMonth);

    /// format month and year in the header
    /// ```
    /// from eg: October 2022
    /// to eg: Oct 2022
    /// ```
    String curMonthStr =
        localizations.formatShortDate(curMonth).replaceAll("1,", "");
    // String curMonthStr = DateFormat("MMM y")
    //     .format(DateTime.parse(localizations.formatMonthYear(curMonth))); TODO: make it customizable
    ...
    DayBasedChangeablePickerState newState = DayBasedChangeablePickerState(
      ...
      isFirstMonth:
          isFirstMonth, //TODO: Change this to false to display all days and months
      isLastMonth:
          isLastMonth, //TODO: Change this to false to display all days and months
    );
